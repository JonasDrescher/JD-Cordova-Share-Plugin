...
- (void)canShareVia:(CDVInvokedUrlCommand*)command {
  ...
  if ([@"sms" caseInsensitiveCompare:via] == NSOrderedSame && [self canShareViaSMS]) {
    ...
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
  }else if ([@"pinterest" caseInsensitiveCompare:via] == NSOrderedSame && [self canShareViaPinterest]) {
      pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
...
- (bool)canShareViaPinterest {
    return [[UIApplication sharedApplication] canOpenURL: [NSURL URLWithString:@"pinterest://app"]]; // requires whitelisting on iOS9
}
...
- (void)shareViaPinterest:(CDVInvokedUrlCommand*)command {
...
/*
    [self.commandDelegate runInBackground:^{ //avoid main thread block  especially if sharing big files from url
        ...
        UIActivity *activity = [[UIActivity alloc] init];
        NSArray *applicationActivities = [[NSArray alloc] initWithObjects:activity, nil];
//        NSArray *app = @[UIActivityTypePostToFacebook, @"pinterest.ShareExtension"];
